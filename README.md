...
For the baseline model, I went with a simple linear regression after splitting the data points into training and test set with the 80-20 split. The two features that are chosen for this model is `calories (#)` and `sodium (PDV)`, which are both continuous quantitative variables. Since, numerical values are used, there is no need to transform any columns, so we kept the numerical as is and put a `LinearRegression` through the `Pipeline`.
